...
class AutoMonitoringTrip(models.Model):
    ...
    def calculate_trips_for_vehicle(self, vehicle_id, date_from=None, date_to=None):
        ...
        A trip starts when:
        - Ignition turns on OR speed > 0 after being stopped
        
        A trip ends when:
        - Ignition turns off OR speed = 0 for > 5 minutes
        """
                ...
                if current_trip is not None:
                    if stop_start is None:
                        ...
                        if stop_duration >= STOP_THRESHOLD_MINUTES:
                            ...
                            current_trip['end_latitude'] = last_moving_point.get('latitude') if last_moving_point else None
                            current_trip['end_longitude'] = last_moving_point.get('longitude') if last_moving_point else None
                            current_trip['end_address'] = last_moving_point.get('address_display_name') if last_moving_point else None
                            end_odometer = (last_moving_point.get('odometer', 0) / 1000.0) if last_moving_point else 0
                            current_trip['distance'] = end_odometer - current_trip.get('start_odometer', 0)
